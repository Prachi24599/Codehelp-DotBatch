...
const whereAmI = function (lat, lng) {
  fetch(
    `https://geocode.xyz/${lat},${lng}?geoit=json&auth=776933478307464037712x72293`
  )
    .then(res => {
    ...
    .then(res => {
      if (!res.ok) throw new Error(`Counytry not found ${res.status}`);
      return res.json();
    })
    .then(data => console.log(data[0]))
    .catch(err => console.error(`${err.message}🔥`));
};
whereAmI(18.52847, 73.85416);
